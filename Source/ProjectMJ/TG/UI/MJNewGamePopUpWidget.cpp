...
void UMJNewGamePopUpWidget::SwitchToInGame()
{
	...
	if (MJGI)
	{
		...
		// 처음 생성하는 세이브 파일일 경우 원하는 기본 스킬들을 가지고 있을 수 있음 
		...
		MJGI->GetPlayerSessionDataRef().CurrentEquippedSkillMap.Add(FGameplayTag::RequestGameplayTag(FName("Skill.Instant")),FGameplayTag::RequestGameplayTag(FName("Skill.Instant.PoisonSlash")));
		...
		MJGI->GetPlayerSessionDataRef().CurrentEquippedSkillMap.Add(FGameplayTag::RequestGameplayTag(FName("Skill.Normal")),FGameplayTag::RequestGameplayTag(FName("Skill.Normal.MeleeAttack")));

		MJGI->GetPlayerSessionDataRef().CurrentOwnedSkillMap.Add(FGameplayTag::RequestGameplayTag(FName("Skill.Passive.DamageUp")));
		MJGI->GetPlayerSessionDataRef().CurrentEquippedSkillMap.Add(FGameplayTag::RequestGameplayTag(FName("Skill.Passive")),FGameplayTag::RequestGameplayTag(FName("Skill.Passive.DamageUp")));
	}
