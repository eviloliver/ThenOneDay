...
void AMJPlayerController::BeginPlay()
{
	...
	// ?ñ∏Î¶¨Ïñº ?óîÏßÑÏùò Ï¥àÍ∏∞?ôî ?àú?Ñú : GameInstance > GameMode > Actor
	// Í∑∏Îü¨ÎØ?Î°? GetSubsystem ?ãú nullptr ?ùÑ Î∞òÌôò?ï† ?ùº??? ?óÜÏß?Îß?, !
	// ?òπ?ãú Î™®Î?? ?ÉÅ?ô©(Î™®Îìà ?àÑ?ùΩ, ?ù¥?ÉÅ?ïú ?ò∏Ï∂? ????ù¥Î∞?, ÎπÑÎèôÍ∏? Î°úÏßÅ Ï§? ?†ëÍ∑? ?ì±)?óê ???ÎπÑÌïò?ó¨ ensure() ?òê?äî UE_LOG Ï∞çÍ∏∞
...
void AMJPlayerController::ChangeToIMCDefault() // showDialogue ÎßàÏ??ÎßâÏóê ?ì§?ñ¥Í∞??ïº ?ï®
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(GetLocalPlayer()))
	{
		Subsystem->AddMappingContext(InputConfigDataAsset->DefaultMappingContext, 0);
...
void AMJPlayerController::ProceedDialogue() 
{
	if (IsTriggered)
	{
		...
		if (DialogueComp->IsDialogueEnd()) // ÎßàÏ??Îß? ????Ç¨?ùºÎ©? imc ?†Ñ?ôò
		{
			ChangeToIMCDefault();
		}
	}
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	UMJInventoryComponent* InventoryComp = Cast<UMJInventoryComponent>(GetWorld());
	 ...
	 {
	 	UE_LOG(LogTemp, Error, TEXT("?ù∏Î≤? Ïª¥Ìè¨?Ñå?ä∏Í∞? ?óÜ???"));
	 }
}
